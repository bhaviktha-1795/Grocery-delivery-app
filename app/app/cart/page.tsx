...
import { Trash2, Plus, Minus, ShoppingCart, X, Percent, DollarSign } from 'lucide-react'
            ...
            <div className="bg-card border border-border rounded-lg p-6 space-y-4 sticky top-20">
              ...
              {couponSuccess && (
                <div className="bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg p-3">
                  <p className="text-xs text-green-700 dark:text-green-300">{couponSuccess}</p>
                </div>
              )}

              {/* Available Coupons */}
              {!appliedCoupon && mockStores[0]?.coupons && (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {mockStores[0].coupons.map((coupon) => (
                      <button
                        ...
                        className="w-full text-left bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/30 rounded-lg p-3 hover:from-primary/20 hover:to-accent/20 transition-colors"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground font-mono truncate">{coupon.code}</p>
                            <p className="text-xs text-muted-foreground">Min: ${coupon.minOrder}</p>
                          </div>
                          <span className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded font-semibold flex-shrink-0">
                            Apply
                          </span>
                        </div>
                      </button>
